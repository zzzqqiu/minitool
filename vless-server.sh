...
_CACHED_IPV4_TS=0
_CACHED_IPV6_TS=0
readonly IP_CACHE_TTL=300  # 缓存有效期 5 分钟（秒）
...
gen_xray_vless_clients() {
    ...
    if [[ -z "$users" ]]; then
        ...
        if [[ -n "$config" && "$config" != "null" ]]; then
            if echo "$config" | jq -e 'type == "array"' >/dev/null 2>&1; then
                ...
                if [[ -n "$uuid" ]]; then
                    if [[ -n "$flow" ]]; then
                        jq -n --arg id "$uuid" --arg e "default@${proto}" --arg f "$flow" '[{id: $id, email: $e, flow: $f}]'
                    else
                        jq -n --arg id "$uuid" --arg e "default@${proto}" '[{id: $id, email: $e}]'
                    fi
                    return
                fi
            else
    ...
    echo "$users" | awk -F'|' -v proto="$proto" -v flow="$flow" -v fport="${filter_port:-}" '
    function trim(s) { gsub(/^[ \t]+|[ \t]+$/, "", s); return s }
    BEGIN { OFS="|" }
    {
        name = trim($1); uuid = trim($2); used = trim($3); quota = trim($4)
        enabled = trim($5); port = trim($6); routing = trim($7)
        if (name == "" || uuid == "" || enabled != "true") next
        if (fport != "" && port != fport) next
        email = name "@" proto
        if (seen[email]++) next
        if (flow != "") {
            printf "{\"id\":\"%s\",\"email\":\"%s\",\"flow\":\"%s\"}\n", uuid, email, flow
        } else {
            printf "{\"id\":\"%s\",\"email\":\"%s\"}\n", uuid, email
        }
...
gen_xray_vmess_clients() {
    ...
    if [[ -z "$users" ]]; then
        ...
        if [[ -n "$config" && "$config" != "null" ]]; then
            ...
            if echo "$config" | jq -e 'type == "array"' >/dev/null 2>&1; then
                ...
                uuid=$(echo "$config" | jq -r '.uuid // empty')
            fi
            if [[ -n "$uuid" ]]; then
                jq -n --arg id "$uuid" --arg e "default@${proto}" '[{id: $id, email: $e, alterId: 0}]'
                return
            fi
        fi
        echo "[]"
    ...
    echo "$users" | awk -F'|' -v proto="$proto" '
    function trim(s) { gsub(/^[ \t]+|[ \t]+$/, "", s); return s }
    {
        name = trim($1); uuid = trim($2); enabled = trim($5)
        if (name == "" || uuid == "" || enabled != "true") next
        email = name "@" proto
        if (seen[email]++) next
        printf "{\"id\":\"%s\",\"email\":\"%s\",\"alterId\":0}\n", uuid, email
    }' | jq -s '.' 2>/dev/null || echo "[]"
}
...
gen_xray_trojan_clients() {
    ...
    if [[ -z "$users" ]]; then
        ...
        if [[ -n "$config" && "$config" != "null" ]]; then
            ...
            if [[ -n "$password" ]]; then
                jq -n --arg pw "$password" --arg e "default@${proto}" '[{password: $pw, email: $e}]'
                return
            fi
        fi
        echo "[]"
    ...
    if [[ -z "$users" ]]; then
        ...
        if [[ -n "$config" && "$config" != "null" ]]; then
            ...
            if [[ -n "$username" && -n "$password" ]]; then
                jq -n --arg u "$username" --arg p "$password" '[{user: $u, pass: $p}]'
                return
            fi
        fi
        echo "[]"
...
db_get_user_days_left() {
    ...
    # 兼容 GNU date (Linux) 和 BSD date (macOS/Alpine busybox)
    local today_sec=""
    local expire_sec=""
    
    if date -d "$(date '+%Y-%m-%d')" '+%s' >/dev/null 2>&1; then
        # GNU date
        today_sec=$(date -d "$(date '+%Y-%m-%d')" '+%s' 2>/dev/null)
        expire_sec=$(date -d "$expire_date" '+%s' 2>/dev/null)
    elif date -j -f '%Y-%m-%d' "$(date '+%Y-%m-%d')" '+%s' >/dev/null 2>&1; then
        # BSD date
        today_sec=$(date -j -f '%Y-%m-%d' "$(date '+%Y-%m-%d')" '+%s' 2>/dev/null)
        expire_sec=$(date -j -f '%Y-%m-%d' "$expire_date" '+%s' 2>/dev/null)
    else
        local today_str=$(date '+%Y-%m-%d')
        if [[ "$today_str" > "$expire_date" ]]; then
            echo "-1"
        elif [[ "$today_str" == "$expire_date" ]]; then
            echo "0"
        else
            local d1=$(echo "$today_str" | awk -F'-' '{print mktime($1" "$2" "$3" 0 0 0")}' 2>/dev/null)
            local d2=$(echo "$expire_date" | awk -F'-' '{print mktime($1" "$2" "$3" 0 0 0")}' 2>/dev/null)
            if [[ -n "$d1" && -n "$d2" && "$d1" =~ ^[0-9]+$ && "$d2" =~ ^[0-9]+$ ]]; then
                echo $(( (d2 - d1) / 86400 ))
            else
                echo ""
            fi
...
# 延迟重载标记：用于批量操作时延迟服务重启
_DEFER_RELOAD=false

# 设置延迟重载模式（批量操作时使用）
# 用法: defer_reload on/off
defer_reload() {
    case "$1" in
        on|true|1)  _DEFER_RELOAD=true  ;;
        off|false|0) _DEFER_RELOAD=false ;;
    esac
}

# 检查是否处在延迟模式，不在时立即重载
_cond_reload_xray() {
    $_DEFER_RELOAD && return 0
...
# 通用流量处理：处理单个核心的流量数据
# 参数: $1=core (xray/singbox), $2=stats_file, $3=notify_percent, $4=alert_thresholds_ref
# 引用传递 alert_thresholds 数组
_process_core_traffic() {
    local core="$1"
    local stats_file="$2"
    local notify_percent="$3"
    shift 3
    ...
    local protocols
    if [[ "$core" == "xray" ]]; then
        protocols=$(db_list_protocols "xray")
    else
        protocols=""
        for p in hy2 tuic anytls; do
            db_exists "singbox" "$p" && protocols+="$p "
        done
    fi
    [[ -z "$protocols" ]] && echo "0|false" && return
    ...
    for proto in $protocols; do
        ...
        if [[ "$core" == "xray" ]]; then
            ...
            while IFS='|' read -r user stat_key; do
                ...
                local uplink=$(grep -F "user>>>${stat_key}>>>traffic>>>uplink " "$stats_file" 2>/dev/null | awk '{print $NF}')
                local downlink=$(grep -F "user>>>${stat_key}>>>traffic>>>downlink " "$stats_file" 2>/dev/null | awk '{print $NF}')
                uplink=${uplink:-0}
                downlink=${downlink:-0}
                local traffic=$((uplink + downlink))
                
                if [[ "$traffic" -gt 0 ]]; then
                    db_update_user_traffic "$core" "$proto" "$user" "$traffic"
                    ((updated++))
                    
                    local quota=$(db_get_user_field "$core" "$proto" "$user" "quota")
                    local used=$(db_get_user_field "$core" "$proto" "$user" "used")
                    _check_user_quota "$core" "$proto" "$user" "$used" "$quota" "$notify_percent" "${alert_thresholds[@]}"
                    _check_user_quota_ret=$?
                    [[ "$_check_user_quota_ret" -eq 2 ]] && need_reload=true
                fi
            done <<< "$mappings"
            continue
        fi
    ...
    echo "${updated}|${need_reload}"
}

# 通用配额检查
# 返回: 0=正常, 1=已告警, 2=超限需重载
_check_user_quota() {
    local core="$1" proto="$2" user="$3" used="$4" quota="$5" notify_percent="$6"
    shift 6
    local -a alert_thresholds=("$@")
    ...
    if [[ "$used" -ge "$quota" ]]; then
        local exceeded_notified=$(db_get_user_alert_state "$core" "$proto" "$user" "quota_exceeded_notified")
        if [[ "$exceeded_notified" != "true" ]]; then
            db_set_user_enabled "$core" "$proto" "$user" "false"
            db_set_user_alert_state "$core" "$proto" "$user" "quota_exceeded_notified" "true"
    ...
    if [[ "$percent" -ge "$notify_percent" ]]; then
        local last_alert=$(db_get_user_alert_state "$core" "$proto" "$user" "last_alert_percent")
        ...
        if [[ "$should_alert" == "true" ]]; then
            ...
            db_set_user_alert_state "$core" "$proto" "$user" "last_alert_percent" "$current_threshold"
        fi
    fi
...
sync_all_user_traffic() {
    ...
    if [[ "$has_xray" == "true" ]]; then
        local xray_result
        xray_result=$(_process_core_traffic "xray" "$tmp_stats" "$notify_percent" "${alert_thresholds[@]}")
        local xray_updated="${xray_result%%|*}"
        local xray_reload="${xray_result##*|}"
        updated=$((updated + xray_updated))
        [[ "$xray_reload" == "true" ]] && need_reload=true
    fi
    
    if [[ "$has_singbox" == "true" ]] && singbox_stats_available; then
        local sb_result
        sb_result=$(_process_core_traffic "singbox" "$tmp_stats" "$notify_percent" "${alert_thresholds[@]}")
        local sb_updated="${sb_result%%|*}"
        local sb_reload="${sb_result##*|}"
        updated=$((updated + sb_updated))
        [[ "$sb_reload" == "true" ]] && need_reload=true
    fi
...
_save_join_info() {
    ...
    for label in V4 V6; do
        ...
        if [[ ${#cmd_parts[@]} -gt 0 ]]; then
            local func_name="${cmd_parts[0]}"
            local func_args=("${cmd_parts[@]:1}")
            link=$("$func_name" "${func_args[@]}")
        else
            link=""
        fi
...
get_ipv4() {
    ...
    if [[ -n "$_CACHED_IPV4" && $((now - _CACHED_IPV4_TS)) -lt $IP_CACHE_TTL ]]; then
        echo "$_CACHED_IPV4"
        return
    fi
...
get_ipv6() {
    local now
    now=$(date '+%s' 2>/dev/null || echo 0)
    if [[ -n "$_CACHED_IPV6" && $((now - _CACHED_IPV6_TS)) -lt $IP_CACHE_TTL ]]; then
        echo "$_CACHED_IPV6"
        return
    fi
